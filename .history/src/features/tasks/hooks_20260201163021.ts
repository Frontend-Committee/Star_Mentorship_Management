...
export const useAdminTaskSubmissions = (taskId: number, options?: { enabled?: boolean }) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<any>(`/admin/task/submissions/{taskId}`); // Speculative query param
      if (response.data && Array.isArray(response.data.results)) {
        return response.data.results as Submission[];
      }
      if (Array.isArray(response.data)) {
...
export const useMemberSubmissions = (id: number, options?: { enabled?: boolean }) => {
  return useQuery({
    queryKey: ['member-submissions', id],
    queryFn: async () => {
      const response = await api.get<any>(`/member/submissions/${id}/`);
      if (response.data && Array.isArray(response.data.results)) {
        return response.data.results as Submission[];
      }
      if (Array.isArray(response.data)) {
...
export const useUpdateSubmission = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<SubmissionCreatePayload> }) => {
      const response = await api.put<Submission>(`/member/submissions/${id}/`, data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['member-submissions'] });
